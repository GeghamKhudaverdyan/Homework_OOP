...
void CheckingAccount::logTransaction(const std::string& type, double amount) const {
	for(int i = 0; i < m_accountTransactions.size(); ++i) {
		if(m_accountTransactions[i]->getTransactionType() == type &&  m_accountTransactions[i]->getAmount() == amount) {
...
void SavingAccount::logTransaction(const std::string& type, double amount) const {
